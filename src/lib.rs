...
pub enum RevoraError {
    ...
    ContractPaused = 35,
    /// Issuer transfer proposal has expired before acceptance.
    IssuerTransferExpired = 36,
    /// Pending admin rotation does not exist.
    NoAdminRotationPending = 37,
    /// Caller is not the proposed new admin for rotation accept.
...
pub enum DataKey {
    ...
    Frozen,
    /// Frozen state for a single offering.
...
impl RevoraRevenueShare {
    ...
    pub fn register_offering(
        ...
        // Register issuer and namespace metadata if not already present.
        Self::ensure_issuer_registered(&env, &issuer);
